...
/*   Updated: 2019/12/16 13:23:30 by bboisset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			string_edition_end(char **str, int pre_fill_len, int end_of_line)
{
	if (!pre_fill_len)
		pre_fill_len = ft_strlen(*str);
	edit_ft_putstr(*str, pre_fill_len, end_of_line);
	free(*str);
	return (pre_fill_len);
}
	...
	while (str[i] != '\0')
	{
		if (str[i] == '%')
		{
			if (!(temp = ft_substr(str, last_addition, i - last_addition)))
				return (-1);
			*total_len += string_edition_end(&temp, 0, 0);
